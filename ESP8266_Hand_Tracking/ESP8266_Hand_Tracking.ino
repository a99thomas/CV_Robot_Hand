...
SoftwareSerial espSerial(5, 4);  // RX, TX for Arduino Nano connection
...
//Changing states for using values from CV or Glove Sensing
void ICACHE_RAM_ATTR IntCallback() {
  ...
  if (state == 0) {
    state = 1;
    digitalWrite(2, HIGH); //LED is on when Using CV
    delay(400); 
  }

  else if (state == 1) {
    state = 0;
    digitalWrite(2, LOW); //LED is off when using glove values
    delay(400);
  }
}
...
void loop() {
  if (state == 0) {
    //Get angle values from Arduino Nano
  ...
  if (state == 1) {
    //Get angles from CV on computer

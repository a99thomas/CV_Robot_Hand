...
cap = cv2.VideoCapture(0) #if your iPhone is setup for continuity camera, you can set this to 1 to ise ot
mpHands = mp.solutions.hands
hands = mpHands.Hands(static_image_mode=False, max_num_hands=1, min_detection_confidence=0.92)
...
def Saturate(angleval, minval, maxval):
    if angleval > maxval:
        angleval = maxval
    if angleval < minval:
        angleval = minval
...
while True:
    ardu = serial.Serial('/dev/cu.usbserial-144140', 115200)  ##INSERT PORT NAME AND BAUD RATE
